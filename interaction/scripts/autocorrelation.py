...
x_fig = figure(title='Signal', width=350, height=300, x_range=(-4.01, 4.01), y_range=(-1, 4))
...
h_fig.line('x', 'y2', source=h_source, line_width=2, line_color='red', line_dash="dotted", legend_label="x(t-t')")
...
y_fig.line('x', 'y', source=y2_source, line_width=2, line_color='green', legend_label="Autocorrelation")
y_fig.legend.location = "top_left"

# Define time shift slider
shift_slider = Slider(title='time shift (s)', value=-4, start=-4, end=4, step=0.01, width=340, height=300)
...
x_selector = Select(title='Signal:', value=options[0], options=options)
...
layout = gridplot([[row(x_fig, h_fig)], 
                   [row(column(x_selector, shift_slider), y_fig)]])
